...
const router = Router();
function pickWeightedPrizeId(prizes) {
...
router.patch('/admin/prizes/:id', requireAdmin, async (req, res) => {
    ...
    const { prizes: prizesCol } = collections();
    const now = new Date();
    const updated = await prizesCol.findOneAndUpdate({ _id: id }, {
        $set: {
...
router.post('/admin/regenerate', requireAdmin, async (req, res) => {
    ...
    if (prizes.length === 0)
        return res.status(409).json({ error: 'no_prizes' });
    const createdTokens = [];
    const now = new Date();
    ...
    while (createdTokens.length < count) {
        ...
        try {
            ...
            if (e && (e.code === 11000 || String(e?.codeName || '') === 'DuplicateKey'))
                continue;
            throw e;
        }
    }
...
export async function ensureSeeded() {
    ...
    if (prizeCount === 0) {
        const now = new Date();
        await prizesCol.insertMany(DEFAULT_PRIZES.map((p) => ({
            _id: p.id,
            label: p.label,
            short: p.short,
            baseCount: p.baseCount,
