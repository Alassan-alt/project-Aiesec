...
async function getLockerWithPrize(token: string) {
...
router.get('/lockers/:token', async (req, res) => {
  ...
  if (token.length < 8) return res.status(404).json({ error: 'not_found' })
...
router.post('/lockers/:token/claim', async (req, res) => {
  ...
  const locker = await lockers.findOne({ _id: token })
  if (!locker) return res.status(404).json({ error: 'not_found' })
...
router.post('/lockers/:token/open', async (req, res) => {
  ...
  const parsed = bodySchema?.safeParse(req.body)
  if (parsed && !parsed.success) return res.status(400).json({ error: 'invalid_body' })
  ...
  try {
    ...
    const again = await lockers.findOne({ _id: token })
    if (!again) return res.status(404).json({ error: 'not_found' })
    if (!again.openedAt) return res.status(500).json({ error: 'open_failed' })
    return res.json({
      token,
